...
from model import VGG16Perceptual, EncoderF4X4 
import numpy as np
import random
...
def parse():
    ...
    # 5 --> 32, 4 --> 16, ...
    parser.add_argument('--num_feat_layer', type=int, default=1)
...
def main(args):
    print(args)
    log_name = make_log_name(args, 'encoder_f_4x4')
    ...
    encoder = EncoderF4X4(in_ch).to(DEV)
        ...
        for i, (x, _) in enumerate(tqdm(dataloader)):
            ...
            if i % args.interval_save == 0:
                ...
                with torch.no_grad():
                    ...
                    writer.flush()
                    torch.save(encoder.state_dict(), './encoder_4x4.ckpt') 
