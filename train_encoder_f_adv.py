...
def parse():
    ...
    parser.add_argument("--b1_d", type=float, default=0.5, help="adam: decay of first order momentum of gradient")
    parser.add_argument("--b2_d", type=float, default=0.999, help="adam: decay of first order momentum of gradient")
...
def main(args):
    print(args)
    log_name = make_log_name(args, 'encoder_f_adv_01')

    if args.seed >= 0:
        set_seed(args.seed)

    # Logger
    path_log = 'runs_adv/' + log_name
    ...
    for epoch in range(args.num_epoch):
        for i, (x, _) in enumerate(tqdm(dataloader)):
            ...
            if args.loss_adv:
                ...
                label_real = torch.full((args.size_batch,), real_label, 
                        dtype=torch.float).to(DEV)
                prop = discriminator(f).view(-1)
                # prop_fake = prop.clone().detach().mean()
                loss_g = bce_fn(prop, label_real)
            ...
            optimizer_g.step()
            # discriminator
            if args.loss_adv and num_iter % 1 == 0:
                ...
                label_real = torch.full((args.size_batch,), real_label, 
                        dtype=torch.float).to(DEV)
                label_fake = torch.full((args.size_batch,), fake_label, 
                        dtype=torch.float).to(DEV)

                prop = discriminator(f.detach()).view(-1)
                prop_fake = prop.clone().detach().mean()
                fake_loss = bce_fn(prop, label_fake)

                prop = discriminator(f_real.detach()).view(-1)
                prop_real = prop.clone().detach().mean()
                real_loss = bce_fn(prop, label_real)

 
                # rt = 0.8
                # loss_d = real_loss * (1 - rt)  + fake_loss * rt
                loss_d = (real_loss + fake_loss) * 0.001
            ...
            if i % args.interval_save == 0:
                ...
                writer.add_scalar('A_prop_real', prop_real.item(), num_iter)
                writer.add_scalar('A_prop_fake', prop_fake.item(), num_iter)
                with torch.no_grad():
