...
from model import VGG16Perceptual, EncoderF8X8 
import numpy as np
import random
...
def parse():
    ...
    # 5 --> 32, 4 --> 16, ...
    parser.add_argument('--num_feat_layer', type=int, default=3)
...
def main(args):
    print(args)
    log_name = make_log_name(args, 'encoder_f_8x8')
    ...
    encoder = EncoderF8X8(in_ch).to(DEV)
        ...
        for i, (x, _) in enumerate(tqdm(dataloader)):
            ...
            if i % args.interval_save == 0:
                ...
                with torch.no_grad():
                    ...
                    writer.flush()
                    torch.save(encoder.state_dict(), './encoder_8x8.ckpt') 
