...
class Discriminator_Fv3(nn.Module):
    def __init__(self):
        super().__init__()
...
class Discriminator_Fv2(nn.Module):
    def __init__(self):
        super().__init__()
...
class EncoderF4X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.avg_pool2d(x, [2, 2])  # 16,  16
        x = F.relu(self.block4(x), True)  # 16,  16, 1024
        x = F.avg_pool2d(x, [2, 2])  # 8, 8 
        x = F.relu(self.block5(x), True)  # 16,  16, 1024
        x = F.avg_pool2d(x, [2, 2])  # 8, 8 
        x = self.block6(x)  # 16, 1024
        return x


class EncoderF8X8(nn.Module):
    def __init__(self, in_ch=3, nc=128, cls_ch=128, k_sz=3):
        super().__init__()
        self.layers = []
        self.input_conv = nn.Conv2d(in_ch, 1 * nc, k_sz, 1, 1)  # 256, 256, 128
